...
namespace ConfuserDeobfuscator.Engine.Routines._1._9L
{
    public class ConstantsDecryption : DeobfuscationRoutine19R
    {
        ...
        {
            var lzmaType =
                Ctx.Assembly.ManifestModule.Types.FirstOrDefault(
                    t => ((string) t.Name).StartsWith("Lzma") && t.HasNestedTypes && t.NestedTypes.Count == 6);

            if (lzmaType == null)
            {
                Ctx.UIProvider.WriteVerbose("No 1.9L encryption detected");
                return false;
            }

            RoutineVariables.Add("lzmaType", lzmaType);
        ...
        public override void CleanUp()
        {
            ...
            var lzmaType = RoutineVariables["lzmaType"] as TypeDef;
            ...
            if (lzmaType != null)
            {
                Ctx.UIProvider.WriteVerbose("Removed Lzma type: {0}", 2, true, lzmaType.Name);
                //TODO: Find out why this can't be removed normally..
                // Ctx.Assembly.ManifestModule.Types.Remove(lzmaType);
                lzmaType.Name = "SoonToBeRemoved:)";
            }
        }
                                            ...
                                            List<Tuple<Instruction, MethodDef>>>> demutatedDecryptors)
        {
            ...
            if (defMd != null)
            {
                ...
                if (key.Length != 32) // Doesn't read same signature before and after anti-tamper
                    key = Ctx.OriginalMD.ReadBlob(resDec.Key0D.GetUInt() ^ ((MethodDef)RoutineVariables["cctor"]).MDToken.ToUInt32()); // .cctor should be at RID 1
        ...
        private static void PopulateKeys(Decryptor decryptor, MethodDef method)
        {
            ...
            decryptor.Key0I = method.Body.Instructions.GetOperandAt<int>(x => x.IsLdcI4(), 1);
            decryptor.Key0D =
